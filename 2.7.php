...
def print_colorful_box():
    reset = '\033[0m'
    text_lines = ["Tool: Gửi tin nhắn Facebook(War)"]
...
class Messenger:
    ...
    def send_message(self, recipient_id, message, mention_id=None, mention_name=None):
        ...
        if mention_id and mention_name:
            tag = f"@{mention_name}"
            data["body"] = f"\u200E{tag} {message}"
            data["profile_xmd[0][offset]"] = 1
            data["profile_xmd[0][length]"] = len(tag)
            data["profile_xmd[0][id]"] = mention_id
...
def send_messages_thread(messenger, recipient_id, message_list, delay, mention_id=None, mention_name=None):
    while True:
        raw_entry = random.choice(message_list)
        parts = [m.strip() for m in raw_entry.split(',') if m.strip()]
        for message in parts:
            success = messenger.send_message(recipient_id, message, mention_id, mention_name)
...
def main():
    ...
    if box_choice == '1':
        ...
        if not os.path.exists(box_file): return
    ...
    if choice == '1':
        ...
        mention_id = input(f"{vang}Nhập ID người cần tag: {xanh_cyan_dam}").strip()
        mention_name = input(f"{vang}Nhập tên hiển thị để tag: {xanh_cyan_dam}").strip()
        message_list = [
    f"sua di {mention_name}",
    f"co len con {mention_name}",
    f"son hang len em {mention_name}",
    f"sao m yeu v {mention_name} ",
    f"co ti nua {mention_name}",
    f"sao kia cham a {mention_name}",
    f"hang hai len ti chu {mention_name}",
    f"toi sang di {mention_name}",
    f"co gang ti con cho {mention_name}",
    f"yeu v con {mention_name}",
    f"con cho {mention_name} co de",
    f"sao m cam kia {mention_name}",
    f"ga v {mention_name}",
    f"may so a k dam chat hang ak {mention_name}",
    f"m ga ma {mention_name}",
    f"may ngu ro ma {mention_name}",
    f"con {mention_name} an hai ma",
    f"cai cun ak {mention_name}",
    f"may con gi khac ko vay {mention_name}",
    f"hoc dot nen nhay dot ak {mention_name}",
    f"co ti di em {mention_name}",
    f"meu a {mention_name}",
    f"sao meu kia {mention_name}",
    f"tao da cho m meu dau {mention_name}",
    f"va le di con {mention_name} dot",
    f"sao kia {mention_name}",
    f"tu bo r a {mention_name}",
    f"manh me ti di con {mention_name}",
    f"co len con cho {mention_name} ngu",
    f"😆 cay tao a con di {mention_name}",
    f"so tao a {mention_name}",
    f"sao cham roi kia {mention_name}",
    f"cay lam phai kh {mention_name}",
    f"{mention_name} ot anh cmnr",
    f"may con choi a {mention_name}",
    f"lam ti keo de {mention_name}",
    f"moi do da met r ha {mention_name}",
    f"sao ga ma son v {mention_name}",
    f"son nhu luc dau cho tao di con {mention_name} dot",
    f"sao duoi roi kia {mention_name}",
    f"ai cho m nhai vay {mention_name}",
    f"cay lam r a {mention_name}",
    f"tu bo di em {mention_name}",
    f"may nghi may lam t cay dc ha {mention_name}",
    f"m dang cay ma {mention_name}",
    f"tao dang hanh m ma {mention_name}",
    f"keo nhay kg ay {mention_name}",
    f"con mo coi {mention_name}",
    f"co len {mention_name} oc cho",
    f"hang tiep di {mention_name}",
    f"toi sang k em {mention_name}",
    f"met roi ha {mention_name}",
    f"speed ti dc ko {mention_name}",
    f"m ga ma {mention_name}",
    f"thay phien a {mention_name}",
    f"tui anh thay phien ban vo loz me con {mention_name} ma kaka",
    f"so tao a con cho {mention_name}",
    f"anh win me roi {mention_name} dot",
    f"ga ma hay the hien ha {mention_name}",
    f"con mo coi {mention_name} keo cai ko em",
    f"co len de {mention_name}",
    f"sao moi 1 ti ma da cam roi {mention_name}",
    f"yeu vay ak {mention_name}",
    f"sao kia {mention_name}",
    f"bat luc r ak {mention_name}",
    f"tim cach roi ha {mention_name}",
    f"ai cuu dc m a :)) {mention_name}",
    f"anh ba cmnr ma {mention_name}",
    f"sao m ga vay {mention_name}",
    f"hang le cho tao di {mention_name}",
    f"con mo coi {mention_name}",
    f"on k em {mention_name}",
    f"bat on roi a {mention_name}",
    f"may oc a con cho {mention_name}",
    f"me may ngu a {mention_name}",
    f"bu cac cha m k em {mention_name}",
    f"mo coi {mention_name} cay anh ha",
    f"me m dot tu roi a {mention_name}",
    f"phe vay {mention_name}",
    f"m on k {mention_name}",
    f"mo coi kia {mention_name}",
    f"sao v so r a {mention_name}",
    f"co gang ti em {mention_name}",
    f"cay cu lam r ha {mention_name}",
    f"dien dai di em {mention_name}",
    f"chay nuoc dai bo r a em {mention_name}",
    f"sao kia dung so anh ma {mention_name}",
    f"may run a:)) {mention_name}",
    f"thg {mention_name} mo coi",
    f"cay tao lam ha {mention_name}",
    f"lgbt len phim ngu ak em {mention_name}",
    f"thg cac giet cha mang me {mention_name}",
    f"sua manh eii {mention_name}",
    f"may chet r a:)) {mention_name}",
    f"sao chet kia {mention_name}",
    f"bi t hanh nen muon chet a {mention_name}",
    f"con {mention_name} loz ngu kaka",
    f"sao kia {mention_name}",
    f"manh len kia {mention_name}",
    f"yeu sinh ly a {mention_name}",
    f"sua de {mention_name}",
    f"cay a {mention_name}",
    f"hang de {mention_name}",
    f"con ga {mention_name}",
    f"phe vat {mention_name}",
    f"oc cho {mention_name}",
    f"me m bi t du hap hoi kia con {mention_name}",
    f"on ko em {mention_name}",
    f"bat on ak {mention_name}",
    f"o kiaaa sao vayy {mention_name}",
    f"hang hai de {mention_name}",
    f"chay ak {mention_name}",
    f"so ak {mention_name}",
    f"quiu luon roi ak {mention_name}",
    f"may dot ak {mention_name}",
    f"cac ngu {mention_name}",
    f"chay de {mention_name}",
    f"chat hang len {mention_name}",
    f"co len {mention_name}",
    f"{mention_name} mo coi",
    f"cn cho ngu {mention_name}",
    f"oc cac {mention_name}",
    f"di du {mention_name}",
    f"du kia {mention_name}",
    f"cun v {mention_name}",
    f"r luon con {mention_name} bi ngu roi",
    f"met r am {mention_name}",
    f"kkakak",
    f"sao du {mention_name}",
    f"cac con {mention_name}",
    f"ngu kia {mention_name}",
    f"chat manh de {mention_name}",
    f"hang ee {mention_name}",
    f"clm thk oc cho {mention_name}",
    f"sua chay de {mention_name}",
    f"sua manh eei {mention_name}",
    f"may oc a con {mention_name}",
    f"tao cho m chay a {mention_name}",
    f"con mo coi {mention_name}",
    f"may chay a con di lon {mention_name}",
    f"sua de {mention_name}",
    f"con phen {mention_name}",
    f"bat on ho {mention_name}",
    f"s do  {mention_name}",
    f"sua lien tuc de {mention_name}",
    f"moi tay ak {mention_name}",
    f"choi t giet cha ma m ne {mention_name}",
    f"hang xiu de {mention_name}",
    f"th ngu {mention_name}",
    f"len daica bieu ne {mention_name}",
    f"sua chill de {mention_name}",
    f"m thich du ko da {mention_name}",
    f"son hang dc kg {mention_name}",
    f"cam chay nhen {mention_name}",
    f"m mau de {mention_name}",
    f"duoi ak {mention_name}",
    f"th ngu {mention_name}",
    f"con {mention_name} len day anh sut chet me may",
    f"m khoc ak {mention_name}",
    f"sua lien tuc de {mention_name}",
    f"thg {mention_name} cho dien",
    f"bi ngu ak {mention_name}",
    f"speed de {mention_name}",
    f"cham v cn culi {mention_name}",
    f"hoang loan ak {mention_name}",
    f"bat on ak {mention_name}",
    f"run ak {mention_name}",
    f"chay ak {mention_name}",
    f"duoi ak {mention_name}",
    f"met r ak {mention_name}",
    f"sua mau {mention_name}",
    f"manh dan len {mention_name}",
    f"nhanh t cho co hoi cuu ma m ne {mention_name}",
    f"cam mach me nha {mention_name}",
    f"ao war ak {mention_name}",
    f"tk {mention_name} dot v ak",
    f"cham chap ak {mention_name}",
    f"th cho bua m sao v {mention_name}",
    f"th dau buoi mat cho {mention_name}",
    f"cam hoang loan ma {mention_name}",
]
    ...
    for messenger in messengers:
        t = threading.Thread(target=send_messages_thread, args=(messenger, recipient_id, message_list, delay, mention_id, mention_name))
        t.daemon = True
        threads.append(t)
        t.start()
